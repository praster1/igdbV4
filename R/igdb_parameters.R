											...
											)
{
  ...
  if(search != "" & ids != "") 
  {
		stop(	sprintf(	error = "Param error!",	cause = "cannot use 'search' & 'ids' at the same time"),	call. = FALSE	)
  }

  if(search != "")	{	result = paste("?search=", search, sep = "")	}

  if(ids != "")		{	result = ids	}

  if(fields != "")	{	result = paste(result, resultQuery(fields, result, "fields"), sep = "")	}

  if(limit != 10)	{	result = paste(result, resultQuery(limit, result, "limit"), sep = "")	}

  if(offset != 0)	{	result = paste(result, resultQuery(offset, result, "offset"), sep = "")	}

  if(order != "")	{	result = paste(result, resultQuery(order, result, "order"), sep = "")	}

  if(expand != "")	{	result = paste(result, resultQuery(expand, result, "expand"), sep = "")	}

  filterLength = length(filter)
  if(filterLength > 0) 
  {
		for (f in 1:filterLength) {	result = paste(result, resultQuery(filter[f], result, "filter"), sep = "")	}
  }
